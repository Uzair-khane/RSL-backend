...
function changeStatus(id, table) {
	$.ajax({
		...
		success: function(response){
			$.growl.notice1({
                title: "Success!",
                message: response.message == undefined ? 'Something went wrong, or access was denied.':response.message
            });
		},
		error: function(xhr, errmsg, err) {
			$.growl.error1({
                title: "Error!",
                message: errmsg == undefined ? 'Something went wrong, or access was denied.':errmsg
            });
		}
	})
}
...
function changeTop(id, table) {
	$.ajax({
		url: `/ap/general/top-status`,
		type: 'POST',
		data: { id: id, source_table: table },
		success: function(response){
			$.growl.notice1({
